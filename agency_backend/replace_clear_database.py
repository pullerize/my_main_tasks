#!/usr/bin/env python3
"""
Скрипт для замены функции clear_database на упрощённую версию
"""
...
for i, line in enumerate(lines):
    if '@app.delete("/admin/clear-database")' in line:
        start_line = i
    elif start_line is not None and '# Endpoint для проверки синхронизации' in line:
        end_line = i - 1
        break

if start_line is None or end_line is None:
    print("[ERROR] Не удалось найти границы функции")
    exit(1)
...
print(f"[OK] Функция clear_database заменена (строки {start_line+1}-{end_line+1})")